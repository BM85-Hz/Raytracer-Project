...
{
    //std::cout << uv;
    // Wrap texture coordinates to handle values outside the range [0, 1]
    //int u = wrap(0, 1);
    //int v = wrap(0, 1);
    ...
    // Needs typecasting because uv isn't an int, duh
    int i = int(wrapped[0] * width);
    int j = int(wrapped[1] * height);

    // Retrieve color as computed index of pixel in 1D texture array (no need since it's in row-major order)
